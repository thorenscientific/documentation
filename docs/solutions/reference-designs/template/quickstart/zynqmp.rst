...
- Instructions on how to choose the boot files from the SD card can be found in
  ...
  :external+adi-kuiper-gen:ref:`hardware-configuration`.
- Instructions on how to manually build the boot files from source can be found
  here:

   - :ref:`linux-kernel zynqmp`
   - :external+hdl:ref:`template_project` build documentation. More HDL build
...
#. Connect the :adi:`EVALUATION BOARD` FMC board to the
...
.. Replace here the boot log from the setup
...
.. Specify the name of the IIO device and the name of the channels from IIO Oscilloscope
...
- no-OS project: :git-no-os:`projects/template`

Instructions on how to build the boot files from source can be found here:

.. - :external+no-OS:doc:`projects/rf-transceiver/template`. More no-OS build
.. details at :external+no-OS:doc:`build_guide`.

- :external+hdl:ref:`template_project`. More HDL build details at
