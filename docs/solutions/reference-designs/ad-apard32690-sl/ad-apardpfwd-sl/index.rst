...
.. figure:: ad-apardpfw-sl_top-angle-evaluation-board.png
   ...
   AD-APARDPFW-SL Angle View

The :adi:`AD-APARDPFW-SL` is a 10BASE-T1L power forwarder
with Single Pair Power over Ethernet (SPoE) for development of field devices and
applications on an :adi:`AD-APARD32690-SL <AD-APARD32690-SL>` platform board.
...
.. figure:: apard-pfwd-top-with-labels.png
   ...
   AD-APARDPFW-SL Primary Side

The :adi:`AD-APARDPFW-SL` uses 2 10BASE-T1L ports. One port
...
.. figure:: apard-pfwd-bottom-with-labels.png
   ...
   AD-APARDPFW-SL Secondary Side

SPoE PD Power Class Selection (JP1 and JP2)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default, the :adi:`LTC9111` SPoE PD
controller included in the :adi:`AD-APARDPFW-SL` circuit is configured for
...
.. warning::
   ...
   The :adi:`AD-APARDPFW-SL` evaluation board is not
...
- :adi:`AD-APARDPFW-SL` Circuit Evaluation Board
- :adi:`AD-APARD32690-SL <AD-APARD32690-SL>`
...
:adi:`AD-APARDPFW-SL` evaluation board and the
...
:adi:`AD-APARDPFW-SL` evaluation board
and ping the :adi:`AD-APARD32690-SL <AD-APARD32690-SL>`:

#. Ensure that the jumpers and switches of the
   :adi:`AD-APARDPFW-SL` are configured to the default settings.

#. Connect the :adi:`AD-APARDPFW-SL` circuit evaluation
...
#. Wait for the **DS1** LED on the
   :adi:`AD-APARDPFW-SL` evaluation board and the **DS1**
...
#. Now you can ping the device to see if the connection is working properly.
   Open a terminal on your host PC connect to the :adi:`AD-RPI-T1LPSE-SL <AD-RPI-T1LPSE-SL>` 
   through SSH:
...
.. tip::
   ...
   videos, and more when you :adi:`register <AD-APARDPFW-SL?&v=RevB>` your
   hardware.
